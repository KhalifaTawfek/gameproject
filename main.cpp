...
int main()
{
    ...
    Mesh playerHand = loader.loadObj("Resources/Models/gun.obj", woodTextures);
    Mesh flashlightModel = loader.loadObj("Resources/Models/flash.obj", flashTextures);
    Mesh bulletModel = loader.loadObj("Resources/Models/bullet.obj", woodTextures);
    ...
    while (!window.isPressed(GLFW_KEY_ESCAPE) && glfwWindowShouldClose(window.getWindow()) == 0)
    {
        ...
        if (showInventory) {
            ...
            if (ImGui::Button("Zombie")) currentItem = 4;
            ImGui::End();
        }

        ImGui::SetNextWindowPos(ImVec2(10, 10));
        ImGui::SetNextWindowSize(ImVec2(300, 150));
        ImGui::Begin("HUD", NULL, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoBackground);
        ImGui::Text("Player HP: %d / %d", playerHP, maxPlayerHP);
        ImGui::ProgressBar((float)playerHP / (float)maxPlayerHP, ImVec2(180, 20), "");
        ...
        if (currentItem == 1) {
            glm::vec3 renderPos = camera.getCameraPosition() + (camFront * 2.0f) + (camRight * 0.5f) - (camUp * 0.5f);
            glUniform3f(glGetUniformLocation(shader.getId(), "objectColor"), 0.8f, 0.8f, 0.8f);
            glActiveTexture(GL_TEXTURE0); glBindTexture(GL_TEXTURE_2D, texFlash); glUniform1i(glGetUniformLocation(shader.getId(), "texture_diffuse1"), 0);
            DrawMesh(flashlightModel, renderPos, glm::vec3(0.15f), true);
        }
...
void updateGameLogic() {
    ...
    static bool zomieSpawned = false;

    if (currentItem == 4 && !zombieSpawned) {
        int type = rand() % 3 + 1;
